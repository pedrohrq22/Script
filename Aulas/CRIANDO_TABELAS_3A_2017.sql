...
--DROP TABLESPACE TABELAS_3A_2017 INCLUDING CONTENTS AND DATAFILES;
CREATE TABLESPACE TABELAS_3A_2017 
DATAFILE 'D:\ORACLE\TABELAS_3A_2017.DBF' SIZE 10M
...
--NA TABLESPACE TABELAS_3A_2017.
--SE O USUÁRIO ABAIXO JÁ FOI CRIADO NA AULA ANTERIOR, RODE O COMANDO AO LADO: 

--DROP USER usuario3A cascade;

CREATE USER usuario3A IDENTIFIED BY 4321 
DEFAULT TABLESPACE TABELAS_3A_2017 
TEMPORARY TABLESPACE TEMP
QUOTA 5M ON TABELAS_3A_2017;

--FORNECER PERMISSÕES AO USUÁRIO: 

--1. PARA PERMITIR CONEXÃO COM O BANCO DE DADOS
GRANT CONNECT TO usuario3A;
GRANT CREATE SESSION TO USUARIO3A;

--2. PARA PERMITIR A CRIAÇÃO DE TABELAS
GRANT ALL PRIVILEGES TO usuario3A;
GRANT CREATE TABLE TO USUARIO3A;
...
CONSTRAINT PK_CARGO1 PRIMARY KEY (CODIGO),
CONSTRAINT CHK_SETOR_CARGO1 CHECK (SETOR IN ('INFORMÁTICA',
'FINANCEIRO','VENDAS','COMPRAS','TREINAMENTO')))
TABLESPACE TABELAS_3A_2017;
...
CONSTRAINT FK_COD_CLI_DEP FOREIGN KEY (COD_CLI) REFERENCES CLIENTE (CODIGO), --Regra de restrição de chave estrangeira, isto é, o código do cliente a quem pertence o dependente
