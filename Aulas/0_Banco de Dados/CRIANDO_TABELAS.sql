...
CONSTRAINT CHK_SETOR_CARGO CHECK (SETOR IN ('INFORMÁTICA','FINANCEIRO','VENDAS','COMPRAS')))
...
CONSTRAINT FK_CARGO_FUNCIONARIO FOREIGN KEY (COD_CARGO) REFERENCES CARGO (CODIGO), --Regra de restrição de chave estrangeira
...
CONSTRAINT PK_COD_DEP_CLI PRIMARY KEY (COD_DEP_CLI),
CONSTRAINT FK_CLIENTE FOREIGN KEY (COD_CLI) REFERENCES CLIENTE (CODIGO),
CONSTRAINT CHK_SEXO_DEP_CLI CHECK (SEXO IN ('F','M'))

)TABLESPACE TABELAS_3B_2017;

SELECT * FROM DEP_CLI

INSERT INTO DEP_CLI (COD_DEP_CLI, COD_CLI, NOME, CPF, EMAIL, SEXO) VALUES(1,1,'Pedro Henrique', '47958664818', 'Pedro_4185@hotmail.com','M');
